...
leader to provide a stable access point; east-west traffic flows over a mandatory
**encrypted WireGuard mesh**.

Key 2.0 point: **every node runs the FastAPI backend** with the same control loop;
**leadership** (a Consul session lock) designates the single node that acts. No node has a
fixed role — a promoted survivor takes over everything (placement, VIP, ingress).
...
(`ConsulStore.acquire_leadership`); **only the leader acts**. With the `FileStore` (single
...
2. **`apply_cluster`** — reads the cluster manifest + live nodes, schedules placement, and
...
|---|---|---|
| **Agent** | `lib/node.sh` (on each node's host) | Renews its Consul lease, syncs its sub-manifest, reconciles it with the normal engine, publishes its meta + backends, applies the WireGuard mesh, and reconciles the VIP. |
| **Control loop** | FastAPI backend (on every node) | Leader-gated: HPA → placement → mesh. Only one leader acts. |

A node holds both at once (backend co-located with the agent). Typical deployment:
**3 nodes**, each an agent + backend, with **3 Consul servers** for quorum.
...
- **`ConsulStore`** (`consul`) — Consul KV over its HTTP API (stdlib `urllib`, no extra
  dependency). Brings **Raft consensus**, **leader election** (sessions/locks); this is the
...
- **constraint-aware** — honours `node_affinity`, anti-affinity and `max_per_node`; it
...
- **placement-only keys** (`total_replicas`, `hpa_*`, affinity, `max_per_node`…) are
  **stripped** from the per-node section — the agent sees only a plain service;
- a section **with no `health_type`** gets `health_type = none` injected, so a bare cluster
  service (e.g. `image = nginx` deployed from the console) is not **repaired-to-death** (the
  container is healthy while it runs; the ingress independently health-checks each backend);
  an explicit `health_type` always wins;
...
1. **Lease renewal** (`node_lease_renew`) — writes its heartbeat to the store. On failure
   ...
   **suspends its creates/repairs** so it does not double-start workloads the leader is
...
- **Quorum**: with **≥ 3 Consul servers**, the 2/3 Raft quorum survives losing one node.
  ...
  agent binds the VIP**. Consul's quorum guarantees a single leader → **no split-brain**.
- **Liveness**: the leader only schedules onto **live** nodes — heartbeat within the
  `CAELIX_NODE_TTL` (30 s by default, `liveness.py`). A node that stops beating is
  **excluded**, and its *stateless* workloads are **rescheduled onto the survivors**. A
  **draining** node is also excluded (its movable workloads leave; a `pinned` app stays
...
- **Lease-based fencing**: a node that loses its lease stops acting as leader and releases
  the VIP — "the lease is the authority": a partitioned-but-alive node does not conflict
  with rescheduling.
...
registry**: each app with an `autoscale_route` key → a route whose backends are the
...
3. applies **asymmetric hysteresis**: scale **up** when avg > target, scale **down** only
   when avg < target × 0.5 (avoids flapping).
...
reset on failover is harmless (HPA re-evaluates from the live metric).
...
- **users**, **JWT secret**, **single-file config**, **templates**, **Compose stacks +
  `.env`**, **TLS certificates**.
...
3. `dockerd` is exposed over **TCP on the node's private IP** (`tcp://<ip>:2375`, set by
   `node_write_cluster_env`); in production, **restrict it to the WireGuard subnet + mTLS**.
...
- each node generates its **key pair** locally (`node_mesh_keygen`) — **the private key
  never leaves the node**; only `wg_pubkey` + `wg_endpoint` go into the meta;
...
- the agent applies the directives every cycle (`node_mesh_ensure`), **idempotently**
  (hash): it re-applies only if they change **or** if the interface disappeared (e.g. **host
  reboot**), so the mesh **survives reboots**.

The system application (`wg` / `ip`) requires root; `wg` is mandatory on a cluster node (the
...
- **Consul quorum**: `Behavior=delete` session lock → a single leader, no split-brain.
- **Remote Docker endpoint**: in production, **restrict it to the WireGuard subnet + mTLS**
