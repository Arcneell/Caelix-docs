...
A cluster node cannot exist without it: the installer requires `wireguard-tools`
and the agent applies the mesh automatically.

!!! note "Cluster and VIP are opt-in"
...
A cluster service is described in the cluster manifest (via the console, in an app
section of the manifest). Unlike a single-host service, you declare a total number
of replicas that the scheduler spreads across nodes, and an ingress route that the
...
!!! note "Default health = none"
    A service with no explicit `health_type` **defaults to `none`** in a cluster:
    it is not "repaired to death". Crash recovery goes through `create_missing`.
    Declare an explicit probe (`health_type = http`, `health_url`, …) to enable
    monitoring.
...
The scheduler places 3 replicas (1 per node thanks to `anti_affinity`), each
published on `<node>:8080`. The leader's global proxy load-balances `VIP:80`
across these backends and drops dead ones. The service is then reachable at:
...
To adjust `total_replicas` automatically based on CPU load, add to the app
section:
...
`hpa_max`), the scheduler re-places, and the ingress load-balances the new
backends.
...
The configured VIP is published to the store: a promoted node takes it over
...
block the drain, the console offers to force it. On the node:
...
The installer writes `/etc/caelix-cluster.env`, loaded by `caelix-agent.service`.
For manual or advanced tuning:
...
Inspection commands: `caelix node-info`, `caelix node-status`,
`caelix vip-status`.
