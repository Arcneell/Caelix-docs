...
| **3 — Réseau cross-host + ingress dynamique** 🚧 | Maillage WireGuard + ingress (proxy existant) alimenté par Consul. **Livré** : registre de services (`register/deregister_backend`, `backends_for`, `service_apps`) + `build_routes` + `GET /api/cluster/routes` **+ publication des backends par l'agent** (`node_publish_backends` : `CAELIX_NODE_ADDR:port`, gated sur container running ; file+consul). **Reste** : régénération du proxy depuis les routes, maillage WireGuard | **Scale horizontal** (répliques réparties derrière un LB) | en cours |
