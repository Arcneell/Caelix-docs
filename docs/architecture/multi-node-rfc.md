...
| **3 — Réseau cross-host + ingress dynamique** 🚧 | Maillage WireGuard + ingress (proxy existant) alimenté par Consul. **Livré** : registre de services dans le store (`register/deregister_backend`, `backends_for`, `service_apps` sur `FileStore`+`ConsulStore`) + constructeur de routes ingress (`build_routes`) + endpoint `GET /api/cluster/routes`. **Reste** : publication des backends par l'agent, régénération du proxy depuis les routes, maillage WireGuard | **Scale horizontal** (répliques réparties derrière un LB) | en cours |
