...
| **4 — HA du controller** 🚧 | Élection de leader, reschedule sur node-down, anti split-brain. **Livré** : heartbeat de nœud (`node_publish_heartbeat`, file+consul) + détection de liveness (`core/cluster/liveness.py`) + reschedule sur nœuds vivants (`apply_cluster(live_ttl=…)`, `POST /api/cluster/apply?live=1`) + liveness dans `/nodes` et `/status`. **Reste** : élection de leader (session/lock Consul), fencing anti-split-brain (révocation de bail) | **Vrai HA** | en cours |
