...
| **4 — Controller HA** 🚧 | Leader election, reschedule on node-down, anti split-brain. **Delivered**: heartbeat + liveness + reschedule (`apply_cluster(live_ttl=…)`, `/apply?live=1`, `/nodes`+`/status`) **+ leader election** (Consul session/lock `create_session`/`acquire_leadership`/`renew_session`/`current_leader`; `FileStore` single-instance always-leader; `GET /api/cluster/leader`). **Remaining**: split-brain fencing (the agent self-suspends when it loses its lease), leader-gated controller loop (daemon) | **True HA** | in progress |
