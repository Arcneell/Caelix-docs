...
| **3 — Cross-host network + dynamic ingress** 🚧 | WireGuard mesh + ingress (existing proxy) fed by Consul. **Delivered**: service registry in the store (`register/deregister_backend`, `backends_for`, `service_apps` on `FileStore`+`ConsulStore`) + ingress route builder (`build_routes`) + `GET /api/cluster/routes`. **Remaining**: agent publishing its backends, proxy regeneration from the routes, WireGuard mesh | **Horizontal scale** (replicas spread behind an LB) | in progress |
