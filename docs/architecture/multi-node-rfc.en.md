...
| **2 — Single-instance controller + store** 🚧 | Cluster manifest, static scheduler, per-node push. **Delivered**: Python placement core (`core/cluster/` — `schedule`, manifest split, `FileStore` §9, `apply_cluster`) **+ agent↔store loop** (`CAELIX_CLUSTER_STORE`: the agent publishes its meta and adopts the pushed `desired.ini` → multi-host MVP on a shared FS). **Remaining**: Consul backend (same interface as `FileStore`) + controller endpoint/UI | **"Managed multi-host"** (single console over N servers) | in progress |
