...
| **4 — Controller HA** 🚧 | Leader election, reschedule on node-down, anti split-brain. **Delivered**: heartbeat + liveness + reschedule (`apply_cluster(live_ttl=…)`, `/apply?live=1`, `/nodes`+`/status`) **+ leader election** (Consul session/lock; `FileStore` single-instance; `GET /api/cluster/leader`) **+ split-brain fencing** (`node_lease_renew`→`node_agent_cycle`: lease lost ⇒ agent FENCED, skips its reconcile pass; `fenced` exposed in the status). **Remaining**: leader-gated controller loop (a daemon acquiring leadership and running `apply_cluster(live)` periodically) | **True HA** | in progress |
