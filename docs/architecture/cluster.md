...
suit le leader pour offrir un point d'accès stable ; le trafic est-ouest passe par un
**maillage WireGuard chiffré** obligatoire.

Point clé de la 2.0 : **chaque nœud exécute le backend FastAPI** avec la même boucle de
contrôle ; le **leadership** (verrou de session Consul) désigne le seul nœud qui agit.
Aucun nœud n'a de rôle figé — un survivant promu reprend tout (placement, VIP, ingress).
...
un verrou de session Consul (`ConsulStore.acquire_leadership`) ; **seul le leader agit**.
...
2. **`apply_cluster`** — lit le manifeste cluster + les nœuds vivants, planifie le
...
|---|---|---|
| **Agent** | `lib/node.sh` (sur l'hôte de chaque nœud) | Renouvelle son bail Consul, synchronise son sous-manifeste, le réconcilie avec le moteur normal, publie sa meta + ses backends, applique le maillage WireGuard et réconcilie la VIP. |
| **Boucle de contrôle** | backend FastAPI (sur chaque nœud) | Verrouillée par leader : HPA → placement → mesh. Un seul leader agit. |

Un nœud tient les deux à la fois (backend co-localisé avec l'agent). Déploiement typique :
**3 nœuds**, chacun agent + backend, avec **3 serveurs Consul** pour le quorum.
...
- **`ConsulStore`** (`consul`) — Consul KV via son API HTTP (stdlib `urllib`, aucune
  dépendance ajoutée). Apporte le **consensus Raft**, l'**élection de leader**
...
- **contraint** — respecte `node_affinity`, l'anti-affinité et `max_per_node` ; il
  ...
  plutôt que d'échouer. La 1re réplique d'une app sur un nœud garde le nom nu de l'app ;
  les suivantes reçoivent un suffixe `-rN`.
...
- les **clés de placement** (`total_replicas`, `hpa_*`, affinités, `max_per_node`…) sont
  **retirées** de la section par-nœud — l'agent ne voit qu'un service simple ;
- une section **sans `health_type`** reçoit `health_type = none` injecté, pour qu'un
  service cluster nu (ex. `image = nginx` déployé depuis la console) ne soit pas
  **réparé-à-mort** (le conteneur est sain tant qu'il tourne ; l'ingress santé-checke
...
1. **Renouvellement du bail** (`node_lease_renew`) — écrit son heartbeat dans le store.
   ...
   (*self-fence*, `node_fence_set`) : il **suspend ses créations/réparations** pour ne pas
...
La **VIP** (`CAELIX_CLUSTER_VIP`, ex. `10.0.0.10/32`) est une adresse stable qui **suit le
leader** :
...
- **Quorum** : avec **≥ 3 serveurs Consul**, le quorum Raft 2/3 survit à la perte d'un
  ...
  **automatiquement libéré** quand la session du leader expire (crash, partition) ; un
  ...
  unique → **pas de split-brain**.
- **Liveness** : le leader ne planifie que sur les nœuds **vivants** — heartbeat dans le
  TTL `CAELIX_NODE_TTL` (30 s par défaut, `liveness.py`). Un nœud qui cesse de battre est
  **exclu**, et ses charges *stateless* sont **replanifiées sur les survivants**. Un nœud
  en **drain** est aussi exclu (ses charges movibles partent ; une app `pinned` y reste en
  `pending`, ce qui **bloque** la complétion du drain).
- **Fencing par bail** : un nœud qui perd son bail cesse d'agir comme leader et relâche la
  VIP — « le bail fait autorité » : un nœud partitionné mais vivant n'entre pas en conflit
  avec le replanning.
...
**registre de services** : chaque app dotée d'une clé `autoscale_route` → une route dont
...
- Le **hash de config** ne couvre que le **listener** (`listen` / TLS) et ne gouverne que
  son redémarrage — ainsi l'ajout d'un service cluster ne nécessite jamais de relancer le
...
3. applique une **hystérésis asymétrique** : scale **up** quand la moyenne > cible, scale
   **down** seulement quand la moyenne < cible × 0,5 (évite le flapping).
...
sont **en-process** ; une remise à zéro au failover est inoffensive (l'HPA réévalue depuis
la métrique vivante).
...
- **utilisateurs**, **secret JWT**, **config mono-fichier**, **templates**, **stacks
  Compose + `.env`**, **certificats TLS**.
...
- chaque nœud génère sa **paire de clés** localement (`node_mesh_keygen`) — **la clé privée
  ne quitte jamais le nœud** ; seuls `wg_pubkey` + `wg_endpoint` partent dans la meta ;
...
- l'agent applique les directives à chaque cycle (`node_mesh_ensure`), de façon
  **idempotente** (hash) : il ne ré-applique que si elles changent **ou** si l'interface a
  disparu (ex. **redémarrage de l'hôte**), pour que le maillage **survive aux reboots**.
...
- **Quorum Consul** : verrou de session `Behavior=delete` → un seul leader, pas de
  split-brain.
