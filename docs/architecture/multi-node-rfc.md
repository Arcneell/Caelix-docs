...
| **2 — Controller mono-instance + store** 🚧 | Manifest cluster, scheduler statique, push par nœud. **Livré** : cœur de placement Python (`core/cluster/` — `schedule` affinité/anti-affinité/`max_per_node`/`total_replicas`+pending, découpe en sous-manifests, `FileStore` au layout §9, action `apply_cluster`). **Reste** : backend Consul (même interface que `FileStore`) + boucle/endpoint controller + enregistrement nœud depuis `agent/status.json` | **« Multi-hôtes managé »** (console unique sur N serveurs) | en cours |
